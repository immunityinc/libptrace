...
typedef struct pt_handle_process
{
	uint64_t	pid;
	uint64_t	creation_time;
} pt_handle_process_t;

#define PT_HANDLE_PROCESS_INIT(p, c)					\
	((pt_handle_process_t){ .pid = (p), .creation_time = (c) })

#ifdef __cplusplus
extern "C" {
#endif

struct pt_process *pt_handle_process_find(struct pt_core *, pt_handle_t);
