 ...
 * process_x86_64.c
 *
 * libptrace windows x86_64 process management.
 *
...
int pt_windows_wow64_process_init(struct pt_process *process)
{
	...
	pt_windows_process_wow64_set(process, 1);
...
struct pt_process *pt_windows_wow64_process_new(void)
{
	struct pt_process *process;
        ...
        if (pt_windows_wow64_process_init(process) == -1) {
                free(process);
                return NULL;
        }
	...
	LPEXCEPTION_RECORD exception)
{
	...
	if (pt_windows_process_wow64_get(process))
		...
		dr6 = pt_windows_thread_x86_64_get_dr6(thread);
	...
	if (pt_windows_process_wow64_get(process))
		...
		pt_windows_thread_x86_64_set_dr6(thread, dr6);
